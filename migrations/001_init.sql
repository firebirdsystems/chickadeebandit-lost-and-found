...
-- in manifest.json. `status`, `category` and `visibility` are hub built-ins on
-- the encryption skip-list so they can be filtered in SQL.
--
-- `kind` is NOT a built-in (the skip-list has `type`, not `kind`) and has no
-- plaintext suffix, so it shipped encrypted at rest: the index below was dead
-- weight and any `WHERE kind = ?` would have matched nothing, silently. It is
-- now listed in manifest.db_plaintext_columns. Do not filter on a column in SQL
-- without checking it against that list first — ciphertext is AES-GCM with a
-- random IV, so equality never matches and nothing errors.
